...
export class RatingInputComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.ratinginput.data_required_flag == 'T' && this.ratinginput.value) {

      this.ratinginput.isValid = true;

    } else if (this.ratinginput.data_required_flag != 'T') {
      this.ratinginput.isValid = true;
    }
    else {
      this.ratinginput.isValid = false
    }
  }
