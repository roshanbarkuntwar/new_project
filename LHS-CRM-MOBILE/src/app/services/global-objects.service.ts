...
export class GlobalObjectsService {
  ...
  public android_version_numeric = 39;
  ...
  attachmentArr: any = [];
  sidemenuDate: any;
  simpleLoginProcess: boolean = false; //(true)=> for appkey login process && (false)=> jump to direct login page like ARS / jyoti 
  ...
  // parentAppkey = "CPGME";
  // parentAppkey = "VICCOCRMSR";
  // parentAppkey = "KISCOCRMSE";
  // parentAppkey = "MIEPLCRMSE";
  ...
  downloadPdf2(shareType, docDefinition, title) {
    ...
    if (this.platform.is("ios")) {
      let directoryPath = this.file.documentsDirectory;
      this.savePdf(shareType, docDefinition, directoryPath, title);
    }

    else if (this.platform.is("android")) {
      let directoryPath 
      // = this.file.externalApplicationStorageDirectory.split("Android")[0] + "/Download/";
      this.checkStoragePermission().then((res) => {
        if (res == 'success') {
          // alert("res: "+JSON.stringify(res));
            directoryPath = this.file.externalApplicationStorageDirectory;
          // directoryPath = this.file.externalApplicationStorageDirectory.split("Android")[0] + "/Download/";
          // let directoryPath  = this.file.externalRootDirectory;
          // alert('directory 1'+ directoryPath);
          this.savePdf(shareType, docDefinition, directoryPath, title);
        } else {
          this.presentAlert("Please grant storage permission to download file.")
        }
  ...
  savePdf(shareType, docDefinition, directoryPath, title) {
    ...
    pdfMake.createPdf(docDefinition).getBuffer(buffer => {
      if (this.platform.is("android")) {
        this.saveFile(buffer, directoryPath, title, type, ext)
        // alert('directory'+directoryPath);
      } 
      else {
        this.pdfObj = pdfMake.createPdf(docDefinition);
        this.pdfObj.download(title);
  ...
  saveFile(buffer, directoryPath, fileName, typee, ext) {
    // alert('buffer'+buffer);
    this.displayCordovaToast("Please wait download started...");
    let files = fileName + JSON.stringify((Math.floor(Math.random() * (999999 - 100000)) + 100000)) + ext;
    let type = typee;
    var blob = new Blob([buffer], { type: type });
    // alert('blob'+blob);
    this.downloadFile(directoryPath, files, blob, type, buffer)
  }
  ...
  downloadFile(directoryPath, fileName, blob, type, buffer) {
    if (this.platform.is("android") || this.platform.is("ios")) {
      this.file.createFile(directoryPath, fileName, true)
        .then(() => {
          this.file.writeFile(directoryPath, fileName, blob, { replace: true })
            .then(fileEntry => {            
              if (fileEntry) {               
        ...
        // this.file.resolveDirectoryUrl(directoryPath)
        //   .then(dirEntry => {
        //     console.log(directoryPath)
        //     this.file.getFile(dirEntry, fileName, { create: true })
        //       .then(fileEntry => {
        //         alert(fileName)
      ...
      // this.displayCordovaToast("Download complete...");
      // saveAs(new Blob([buffer], { type: type }), fileName);
      // this.displayCordovaToast("Download complete...");
    }
  }
  ...
  // checkStoragePermission() {
  //   return new Promise((resolve, reject) => {
  //     this.androidPermissions.hasPermission(this.androidPermissions.PERMISSION.READ_MEDIA_IMAGES)
  //       .then(status => {
  //         alert(status+'status')
  //         this.androidPermissions.hasPermission(this.androidPermissions.PERMISSION.PERMISSION.WRITE_EXTERNAL_STORAGE).then((writeStatus) => {
  //           if (writeStatus.hasPermission) {
  //             if (status.hasPermission) {
  //               this.file.checkDir(this.file.externalRootDirectory, "Download").then((res) => {
  //                 alert('res'+res)
  ...
  //             }
  //             else {
  //               this.androidPermissions.requestPermission(this.androidPermissions.PERMISSION.PERMISSION.READ_EXTERNAL_STORAGE)
  //                 .then(status => {
  //                   this.androidPermissions.hasPermission(this.androidPermissions.PERMISSION.PERMISSION.WRITE_EXTERNAL_STORAGE).then((writeStatus) => {
  //                   if (status.hasPermission) {
  ...
  //             }
  //           } else {
  //             this.androidPermissions.requestPermission(this.androidPermissions.PERMISSION.PERMISSION.READ_EXTERNAL_STORAGE).then((writeStatus1) => {
  //               if (writeStatus1.hasPermission) {
  //                 if (status.hasPermission) {
  //                   this.file.checkDir(this.file.externalRootDirectory, "Download").then((res) => {
  //                     resolve("success")
    ...
    if (this.plt.is("android") || this.plt.is("ios")) {
      pdfMake.createPdf(docDefinition).getBuffer((buffer => {
        ...
        this.file.writeFile(directoryPath, fileName, blob, { replace: true })
          ...
          .then(fileEntry => {
            // alert(directoryPath +'this is in pdffile send');
            this.displayCordovaToast("Download complete...");
            if (fileEntry) {

              if (fileEntry.isFile) {
                // alert(fileEntry.isFile+ "in the fileentry");
                this.displayCordovaToast("File Generated");
              }
             
              this.file.checkFile(directoryPath, fileName).then((exist => {
                this.filePath.resolveNativePath(directoryPath + fileName).then((pathIs => {
                  // alert(pathIs + "after the checkfile");
                }));
                let email = {
                  to: '',
                  cc: '',
