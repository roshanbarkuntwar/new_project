...
declare var google;
@Component({
  selector: 'app-frame-line-graph',
  templateUrl: './frame-line-graph.component.html',
  styleUrls: ['./frame-line-graph.component.scss'],
})
export class FrameLineGraphComponent implements OnInit {
  @Input() frame: any = {};
  @Input() wsdp: any = {};
  ...
  ngOnInit() {
    this.lineGraph = this.frame.design_control_type;
    if(this.lineGraph){
      this.lineGraphFlag = true;
    }else{
      this.lineGraphFlag = false;
      this.lineGraphValue = this.lineGraph;
    }
    this.getData();
  }
  getData() {
