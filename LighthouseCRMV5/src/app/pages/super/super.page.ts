...
import { ModalController, PopoverController, MenuController, NavController, Platform } from '@ionic/angular';
...
export class SuperPage implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.userDetails) {
      ...
      this.wsud.latitude = this.background.latitude;
      this.wsud.longitude = this.background.longitude;
                  ...
                  if (item.mirror_item_seqid) {
                    for (let fromObj of this.object_mast.Level2) {
                      for (let fromFrame of fromObj.Level3) {
                        ...
                        if (fromFrame.tableRows && fromFrame.tableRows.length > 0 ) {
                          let i = 0;
                          for (let frameLevel4 of fromFrame.tableRows) {
                            for (let fromItemGroup of frameLevel4) {
                              if (fromItemGroup.Level5) {
  ...
  execute_plsql(nextPageInfo, event, clickEventStr) {
    ...
    let callingItm_CB;
    let clickEvent_CB = [];
    if (clickEventStr.indexOf("execute_item_plsql_cb") > -1) {
      let strArr = clickEventStr.split("#");
      for (let c of strArr) {
        if (c.indexOf("execute_item_plsql_cb") > -1) {
  ...
  samePageForm(nextPageInfo, event, clickEventStr) {
    ...
    let mainPage = this.object_mast.Level2[(this.page_no ? this.page_no : 0)].Level3;
    let newPage;
