...
.service('AuthServices', function($q, $http, $rootScope) {
    ...
    function useCredentials(token) {
        userCode = token.split(',')[0];
        userName = token.split(',')[1];
        appType = token.split(',')[2];
        _url = token.split(',')[3];
        entity_code = token.split(',')[4];
        division_data = token.split(',')[5];
        acc_year = token.split(',')[6];
        dept_code = token.split(',')[7];
    ...
    function destroyUserCredentials() {
        ...
        appType = '';
        _url = '';
        entity_code = division_data = acc_year = dept_code = '';
        isAuthenticated = false;
        window.localStorage.removeItem(LOCAL_TOKEN_KEY);
    }

    var login = function(userCode, userName, appType, _url, entityCode,
        divisionData, accYear, deptCode) {
