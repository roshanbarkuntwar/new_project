...
export class FrameTable2Component implements OnInit {
  ...
  itemValueChange(event, rowindex) {
    ...
    if (event.dependent_column_str && event.value) {
      let wsdp = [];
  ...
  calculateData() {
    ...
    this.colSpan = this.thead.length +  this.increamentColspan;
    for (let i = 0; i < (this.thead.length + this.increamentColspan); i++) {
      for (let itemDataArr of this.tableRows) {
        this.colSpan = this.thead.length;
  ...
  downloadExcel(grapgRows) {
    ...
    for (let trows of grapgRows) {
      ...
      for (let trows2 of trows) {
        if (trows2.Level5[0].item_visible_flag == "T") {
          if (trows2.Level5[0].item_type == 'BT') {
          } else {
