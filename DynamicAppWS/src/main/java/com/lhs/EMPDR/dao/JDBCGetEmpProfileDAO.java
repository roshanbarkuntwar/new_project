 ...
 */
public class JDBCGetEmpProfileDAO {

    public Connection connection = null;

    public JDBCGetEmpProfileDAO(Connection connection) {
    ...
    public EmpProfileModel getProfile(String user_code) {
        ...
        String empProfile = "select t.emp_name empName, lhs_utility.get_name('dept_code',t.dept_code) dept_name, (select y.desig_name from emp_desig_mast y where y.desig_code = t.desig_code)desig_name,\n"
                ...
                + "  from emp_mast t,emp_status_mast k where t.emp_code =k.emp_code\n"
                + "  and t.emp_code =(select u.emp_code from user_mast u where u.user_code='" + user_code + "')";

        System.out.println("query = " + empProfile);
        ...
        try {
            ...
            } // Logger.getLogger(JDBCGetEmpProfileDAO.class.getName()).log(Level.SEVERE, null, ex);
            catch (SQLException ex) {
                ex.printStackTrace();
            }
        }
        ...
        try {
            ...
            if (resultSet2 != null && resultSet2.next()) {
                System.out.println("fetching photo" + resultSet2.getRow());
                if (resultSet2.getBinaryStream("photo") != null) {
                    ...
                    System.out.println("photo" + Util.getImgstreamToBytes(inputStream).length);
                } else {
