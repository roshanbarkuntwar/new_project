...
angular.module('nlFramework', [])
        .factory('$nlFramework', ['$nlConfig', '$nlDrawer', '$nlBurger', '$nlRefresh', '$nlToast', '$nlMenu', '$nlFab', '$nlHelpers', '$nlElements',
        ...
        .factory('$nlHelpers', function () {
            var nlHelpers = {
                translate: function (myElement, x, pmX, y, pmY, deg, pmDeg, width, scale, mozieo, opacity) {
                    ...
                    if (mozieo)
                        el.style.msTransform = el.style.MozTransform = el.style.OTransform = 'translateX(' + pmX + x + 'px) translateY(' + pmY + y + 'px) rotate(' + pmDeg + deg + 'deg)';
                },
        ...
        .factory('$nlBurger', ['$nlConfig', '$nlHelpers', '$nlElements', function ($nlConfig, $nlHelpers, $nlElements) {
                var nlBurger = {
                    ...
                    init: function () {
                        ...
                        if (document.getElementById('nlBurger') === null)
                            document.body.insertAdjacentHTML('beforeend', burger);
                        //
        ...
        .factory('$nlDrawer', ['$nlConfig', '$nlBurger', '$nlHelpers', '$nlElements', '$nlFab', function ($nlConfig, $nlBurger, $nlHelpers, $nlElements, $nlFab) {
                var nlDrawer = {
                    init: function (config) {
                        if (config.openCb)
                            nlDrawer.on.show = config.openCb;
                        if (config.closeCb)
                            nlDrawer.on.hide = config.closeCb;
        ...
        .factory('$nlToast', ['$nlConfig', '$nlHelpers', '$nlElements', function ($nlConfig, $nlHelpers, $nlElements) {
                var nlToast = {
                    ...
                    show: function (options) {
                        ...
                        if ($nlConfig.runnigTimeout)
                            clearTimeout($nlConfig.runnigTimeout);
                        ...
                        if (title)
                            $nlElements.toast.innerHTML = title;
