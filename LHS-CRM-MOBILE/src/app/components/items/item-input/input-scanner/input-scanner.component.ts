...
export class InputScannerComponent {
    ...
    public alertController: AlertController, public platform: Platform, public toastController: ToastController) {
      ...
      if ((this.platform.is('android'))){
        this.platform.ready().then((readySource) => {
          ...
          events.destroy('status:dw63ApisAvailable');
          ...
          events.destroy('status:dw64ApisAvailable');
          ...
          events.destroy('status:dw65ApisAvailable');
          ...
          events.destroy('data:activeProfile');
          ...
          events.destroy('data:commandResult');
          ...
          events.destroy('data:enumeratedScanners');
          ...
          events.subscribe('data:scan', (data: any) => {
            ...
            this.inputScanner.value = scannedData;
           // alert("from data:scan   " + JSON.stringify(scannedData))
            this.onChange();
  ...
  onChange() {
    this.inputScanner.isValid = true;
    // alert(JSON.stringify(this.inputScanner));
    if (this.inputScanner.value) {
      this.emitPass.emit(this.inputScanner);
    }
  }
}
