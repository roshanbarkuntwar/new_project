...
angular.module('starter.controllers').controller('addPopulatedLocationEntryCtrl', function ($cordovaGeolocation, AuthServices, $filter,
    ...
    $scope.save = function (rowsOfPopulateData) {

        if (sp_Obj.type == "offlineUpdateEntry") {

            dataServices.updateEntryToLoacalDB(rowsOfPopulateData, l_appSeqNo, sp_Obj.index, $scope.uploadEntryStatus).then(function (data) {
            ...
            dataServices.addEntryToLoacalDB(rowsOfPopulateData, l_appSeqNo, $scope.fieldsTH, $scope.defaultPopulateDataLength, $scope.uploadEntryStatus, entryType).then(function (data) {
                $ionicHistory.goBack(-1);
            }, function (err) {
                $cordovaToast.show('Try Again..', 'long', 'bottom')
            })
    ...
    $scope.dependent_nullable_logic = function (value, column_name) {
        ...
        .service('locationEntryServices', function ($q, $http, $rootScope, pouchDBService, AuthServices, $cordovaToast, $filter) {
            ...
            function setinfo(fields, defaultPopulateData) {
                ...
                //         obj1.column_default_value = obj1.column_default_value.split("#");
                //     }
                ...
                for (var i = 0; i < res.defaultPopulateDataLength; i++) {
                    fields.forEach(function (obj) {
                        ...
                        if (obj.column_desc == "SLNO") {
                            ...
                            if (tempDefaultPopulateData == "undefined" || tempDefaultPopulateData == undefined || tempDefaultPopulateData == "") {
                            } else {
