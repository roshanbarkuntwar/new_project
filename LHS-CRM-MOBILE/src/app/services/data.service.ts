...
export class DataService {
  ...
  constructor(private rendererFactory: RendererFactory2, public http: HttpClient, private camera: Camera, private pouchDBService: PouchDBService,
    ...
    private geolocation: Geolocation, private backgroundMode: BackgroundMode) {
  ...
  postData(url, data) {
    ...
    if (data.wsud) {
      data.wsud.latitude = this.globalObjects.latitude;
      data.wsud.longitude = this.globalObjects.longitude;
    }
  ...
  downloadFile(l_url) {
    return new Promise((resolve, reject) => {
      // this.http.get(l_url,{responseType: 'arraybuffer' , headers: {'Access-Control-Allow-Origin':'*', 'Content-Type':'application/json'}}).subscribe((res)=>{
      this.http.get(l_url, { responseType: "blob" }).subscribe((res) => {
      ...
      this.camera.getPicture(options).then(
        imageData => {
          if (flag == "PHOTOLIBRARY") {
            ...
            let url = imageData.substring(0, imageData.lastIndexOf("/"));
            let filename = imageData.substring(imageData.lastIndexOf("/") + 1);
            this.file.readAsArrayBuffer(url, filename).then((res) => {
              ...
              //  alert(JSON.stringify(blob));
                this.camera.cleanup();
                 this.backgroundMode.disable();
                this.globalObjects.converToBase64(blob).then((res: any) => {
                  resolve(res.split(",")[1]);
