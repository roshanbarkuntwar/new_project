...
public class LOV {
    ...
    public String createLOVsql(Connection c) throws SQLException {
        ...
        sql.append("SELECT select_list_val, 'SELECT DISTINCT ' || COLUMN_NAME   || ','|| DISPLAY_VALUE  || ' FROM ' ||  TABLE_NAME  ||"
                + "' WHERE 1=1 '  FROM VIEW_PORTAL_UPLOAD_TBL_LOV_GEN WHERE UPPER(TRIM( TABLE_NAME  || '.' ||  COLUMN_NAME )) = UPPER('" 
                + REF_LOV_TABLE_COL + "')");
        PreparedStatement ps;
        ResultSet rs;
        try {
            System.out.println("lov sql==> "+sql.toString());
